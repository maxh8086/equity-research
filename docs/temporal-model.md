...
`financial_facts` follows the same shape (migration 0008): `rule_version`
joins its uniqueness key, and `ingest/nse_xbrl` reparses stored XBRL files
into new `financial_filing` and `financial_facts` rows at the file's original
`as_of`. `facts_as_of` takes the newest row per (line item, period) by `as_of`,
then `id`. Rows from before 0008 carry `rule_version = 'pre-0008'`.
